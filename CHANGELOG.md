...
- **`brew` and the fiber handle types.** Beans grew green threads: `brew f(x)`
  starts a child fiber, and `Brew<T>`, `TaskGroup<T>` and `Gate` join the
  builtin types. `brew` highlights as a keyword only where the compiler reads
  one — directly before a call, as a statement or a `let`/`var` initializer.
  Everywhere else it stays the name it is, so the `TaskGroup` method
  `group.brew(...)`, a field or parameter called `brew`, and `brew = brew + 1`
  all keep their ordinary colours. The Tree-sitter grammar gained
  `brew_statement` and `brew_expression`, and because the decision needs a look
  past the word, the external scanner emits the keyword token — a regular lexer
  would have taken `brew` at the start of every statement.
